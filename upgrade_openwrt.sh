...
if [ $model_info_found -eq 0 ] && [ -r /proc/device-tree/model ]; then
    # 备选方法 2: /proc/device-tree/model (适用于某些设备, 特别是 ARM)
     model_dt=$(cat /proc/device-tree/model 2>/dev/null | tr -d '\0') # 读取并移除可能的 null 字节
     if [ -n "$model_dt" ]; then
